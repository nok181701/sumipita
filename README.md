...
> buildId（`next.config.mjs`の`generateBuildId`）はNext.jsのデフォルト通り毎ビルドでランダムになっている。以前はR2 incremental cacheのキー（prefix/buildId/hash）を固定するためbuildIdも固定していたが、それだとコードを変えるデプロイをしたときに「古いビルドのJSチャンクを参照したままのR2キャッシュ」が残り、そのページに新規アクセスが来た瞬間だけ壊れて見える不具合（ChunkLoadError）が実際に発生した。ランダムにしたことで、デプロイのたびに前のbuildId配下のキャッシュは参照されなくなり（新しいbuildIdでしか引かれない）、この不整合が起きなくなった。参照されなくなった古いキャッシュはR2のライフサイクルルール（30日で自動削除）に任せて自然に消える。
